# IRI Dollar Cost Average (DCA) API

## Overview
This repository consolidates **Dollar Cost Average (DCA) arrangement transactions** for annuity products under the **IRI Digital First vision**.

- **Submit DCA Arrangement**
- **Update / Override/Cancel DCA Arrangement**

The initiative modernizes legacy batch/manual DCA processes into RESTful APIs for secure, scalable, and interoperable processing. It leverages industry standards and provides a unified approach for carriers, distributors, and solution providers.

Dollar Cost Averaging (DCA) is an investment strategy where a fixed amount is invested or transferred at regular intervals regardless of market conditions, helping reduce the impact of volatility and eliminate the need for market timing. citeturn10search13turn10search19
...
### 1. Submit DCA Arrangement
Handles scenarios where a new DCA arrangement is submitted.  
**Endpoint:** `/v1/policies/{policyNumber}/arrangements/dollar-cost-average`

### 2. Update / Override / Cancel DCA Arrangement
Handles scenarios where an existing DCA arrangement is updated or overridden.  
**Endpoint:** `/v1/policies/{policyNumber}/arrangements/dollar-cost-average/{arrangementId}`

---
## Supported Transactions

### 1. Submit (Create) DCA Arrangement
...
## Schema Overview
Sample of what most schemas include (Submit and Update DCA arrangements share a common structure with minor differences such as externalArrangementId for Submit and arrangementId for Update):

- **Root Attributes:** effectiveDate, externalArrangementId (Submit only), nsccParticipantId, cusip, actionIndicator.
- **Arrangement Details:** productCode, arrangementType (DOLLAR_COST_AVERAGE), arrangementSubType, frequency, startDate, endDate.
- **Transfer Configuration:** sourceTransferAmountType (AMOUNT/PERCENT), destinationTransferAmountType (AMOUNT/PERCENT).
- **Funds:** transferFromFunds[] and transferToFunds[] with fundId
...
All API operations—across synchronous and asynchronous processing models—adhere to a unified **Error schema**. This ensures consistent error handling, predictable integration behavior, and standardized troubleshooting across all DCA arrangement transactions.
...
- Return a 'DCAResponse' object in the response body, including 'status'(CREATED, REJECTED), and 'correlationId'
- A GET endpoint is available to retrieve the lifecycle status with **HTTP 200 — SUCCESS**.

### Update (PUT) Response
- Return **HTTP 202 Accepted**.
- Include a Location header pointing to /v1/policies/{policyNumber}/arrangement-requests/{requestId}
- Return a 'DCAResponse' object in the response body, including 'status'(ACCEPTED, REJECTED) and 'correlationId'
- A GET endpoint is available to retrieve the lifecycle status with **HTTP 200 — SUCCESS**.
...
DCA arrangement transactions use an asynchronous processing model after initial request submission.  
...
## OpenAPI Specs
Unified Swagger documentation for all DCA arrangement endpoints is available in the `openapi-specs/` folder.

The specifications include:
- Submit (POST) DCA arrangement endpoint definitions
- Update (PUT) DCA arrangement endpoint definitions
